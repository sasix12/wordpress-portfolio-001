...
get_header(); ?>
<?php if(isset($boston_redux_demo['cat_bg']['url']) && $boston_redux_demo['cat_bg']['url'] != ''){?>
<section class="section bg-gray pt-200 bg-img" style="background-image: url('<?php echo esc_url($boston_redux_demo['cat_bg']['url']); ?>');">
<?php }else{?>
		...
		<div class="row section-heading">
			<div class="col-lg-12">
				<h3 class="text-center">
					<?php if(isset($boston_redux_demo['cat_title']) && $boston_redux_demo['cat_title']!=''){?>
						<span><?php echo esc_attr($boston_redux_demo['cat_title']);?></span>
					<?php }else{?>
						<span><?php echo esc_html__( 'Category:', 'boston' ); ?></span>
					<?php } ?>
					<span class="cap"><?php printf( esc_html__( ' %s', 'boston' ), single_cat_title( '', false ) );?></span>
				</h3>
			</div>
		</div>
	</div>
